...
if ($success === true)
{
    ...
    $conn->query("INSERT INTO bz_payment_details(payment_for,order_id,user_id,txn_id,amount,status,txn_date,entry_source,created_at) VALUES('PAYMENT_FOR_PREMIUM_TEMPLATE','".$_POST['razorpay_order_id']."','".$_SESSION['id']."','".$_POST['razorpay_payment_id']."','".$_SESSION['template_payment_amount']."','PAYMENT_SUCCESS','".date("Y-m-d H:i:s")."','web','".date("Y-m-d H:i:s")."')");
    ...
    $conn->query("UPDATE bz_user_profile SET is_payment_done='Y' WHERE id=".$_SESSION['id']);

    $_SESSION['is_payment_done'] = "Y";
    $_SESSION['razorpay_payment_id'] = $_POST['razorpay_payment_id'];
    
    header("Location: ".$_SESSION['base_url']."profile/".base64_encode($_SESSION['template_choosen']));
}
else
{
