...
class Digital_Card_CI extends CI_Controller {	
	...
	public function index($username = "")
	{
        if(!empty($username)) {
            ...
            if($result != false) {
                ...
                $this->session->set_flashdata("message", "error_card");
    ...
    public function get_slider_information() {
      ...
      if($slider_image_data != false){
        $dynamic_index_html .= '<div id="carouselExampleSlidesOnly" class="carousel slide " touch="true" data-ride="carousel" data-interval="1000">
            <div class="carousel-inner" style="display:flex;/*height: 300px*/;width: 100%;background-size: cover;align-items: center;">';
            ...
            foreach($slider_image_data as $row) {
              if(!empty($row->slider_image_path)){
                ...
                if(!empty($row->slider_url))
                  $dynamic_index_html .= '<div class="carousel-item '.$active.'" style="flex-shrink: 0;">
                                          <a href="'.$row->slider_url.'" target="_blank"><img class="" src="'.$row->slider_image_path.'" style="max-width:100%;"  alt="..."></a>
                                      </div>';
                else
                  $dynamic_index_html .= '<div class="carousel-item '.$active.'">
                                          <a href="javascript:void(0);"><img class="" style="max-width:100%;" src="'.$row->slider_image_path.'"  alt="..."></a>
                                      </div>';
                $i++;
              }
                              ...
                              <div class="form-group">
                                <div class="row">
                                  <div class="col-lg-12">   
                                      ...
                                      <div class="form-grmessage_contentoup row">
                                  ...
                                  <div class="col-lg-12">   
                                      ...
                                      <div class="form-group row">
                                          <div class="col col-lg-12 col-md-12"> 
                                              <textarea required class="form-control message_content" cols="50" rows="4" name="message_content">Thank you for contacting '.$_SESSION['display_name'].' - Deceuninck! Please let us know how we can help you</textarea>
                                          </div>
                                      </div>
                                  </div>
                                </div>
    ...
    public function delete_page($page_id) {
        $result = $this->Digital_Card_Model->delete_page($page_id);
        echo $result;  
    }
    
    public function get_accordian_information() {
      header('Access-Control-Allow-Origin: *');
      header('Access-Control-Allow-Methods: POST, GET, OPTIONS');
      $username = $this->input->post("username");
      $accordian_information = $this->Digital_Card_Model->get_accordian_information($username);
      $dynamic_index_html = "";
      
      if($accordian_information != false){
          echo $accordian_information;
      } else {
          echo "false";
      }
    }
